...
JsonValue::JsonValue(const JsonArray& value) : value_(std::make_shared<JsonArray>(value)) {}
JsonValue::JsonValue(const JsonObject& value) : value_(std::make_shared<JsonObject>(value)) {}

bool JsonValue::isNull() const { return std::holds_alternative<JsonNull>(value_); }
bool JsonValue::isBool() const { return std::holds_alternative<JsonBool>(value_); }
bool JsonValue::isNumber() const { return std::holds_alternative<JsonNumber>(value_); }
bool JsonValue::isString() const { return std::holds_alternative<JsonString>(value_); }
bool JsonValue::isArray() const { return std::holds_alternative<std::shared_ptr<JsonArray>>(value_); }
bool JsonValue::isObject() const { return std::holds_alternative<std::shared_ptr<JsonObject>>(value_); }
...
int JsonValue::asInt() const { 
    return static_cast<int>(std::get<JsonNumber>(value_)); 
}
...
std::string JsonParser::stringify(const JsonValue& value, bool pretty) {
    ...
    auto writeValue = [&](const JsonValue& v) {
        if (v.isNull()) {
            oss << "null";
        } else if (v.isBool()) {
